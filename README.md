...
- Use techs as close as possible to the ones discussed about the DS. React of course. Accessibility is not even a question.Responsive is also a no-brainer. From what I remember people use a wide variety of devices (e.g. from laptops to iPads, etc). Not sure the exact browser support though. This would be great. I went with a classic "modern" browser support (see my opinion on it [here](https://raphaelferrand.com/posts/browsers-list-for-design-systems/) ):
...
- Styling
  - I went with pure CSS. Because that's what I prefer. The DS might be using Tailwind, or something else. I went pure CSS. From what I can see the marketing website is also NOT using Tailwind or Tailwind-like utility-class approach.
...
- State management
  - I did not know what is the state management system used in Aize products, and sinc ethe scope of the assignment is limited + we're focusing on the design system, I decided to keep this part as limited and mocked-up as possible.
...
- write Storybook stories to build the variants and document the components
  - add variants for the different states (hover, active, disabled, etc) + option/required
  - add conponent styling
  - add global styling
...
- a button component whould have many variants. One of it is that buttons are not always button, they're sometimes a link. But we sometimes want the same UI for both, and then the purpose of the button should decide what `type` it is (action VS navigation).
